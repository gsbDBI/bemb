...
class TestUtilityParser(unittest.TestCase):
    def test_parser_and_model_creation(self):
        ...
        self.assertTrue(additive_decomposition[0]['coefficient'] == [
                        'intercept1_constant'])
        self.assertTrue(
            additive_decomposition[1]['coefficient'] == ['intercept2_item'])
        self.assertTrue(
            additive_decomposition[2]['coefficient'] == ['intercept3_user'])
        self.assertTrue(
            all(additive_decomposition[i]['observable'] is None for i in range(3)))

        self.assertTrue(
            additive_decomposition[3]['coefficient'] == [
                'alpha_item',
                'beta_user'] and additive_decomposition[3]['observable'] is None)
        self.assertTrue(additive_decomposition[4]['coefficient'] == [
                        'gamma_user'] and additive_decomposition[4]['observable'] == 'user_obs')
        self.assertTrue(additive_decomposition[5]['coefficient'] == [
                        'delta_item'] and additive_decomposition[5]['observable'] == 'item_obs')
        self.assertTrue(
            additive_decomposition[6]['coefficient'] == [
                'comp_user',
                'comp_item'] and additive_decomposition[6]['observable'] == 'user_obs')
...
class TestBEMBFlex(unittest.TestCase):
    ...
    def test_forward_shape(self):
        dataset_list = simulate_choice_dataset.simulate_dataset(
            num_users=num_users, num_items=num_items, data_size=data_size)
        ...
        for return_type, return_scope, deterministic, pred_item in itertools.product(
                ['utility', 'log_prob'], ['item_index', 'all_items'], [True, False], [True, False]):
            if not pred_item:
                # generate fake binary labels.
                batch.label = torch.LongTensor(
                    np.random.randint(2, size=len(batch)))
            ...
            if (return_scope == 'item_index') and (deterministic):
                self.assertEqual(output.shape, (len(batch),))
            elif (return_scope == 'all_items') and (deterministic):
...
class TestBEMBFlexV2(unittest.TestCase):
    ...
    def test_prediction_shapes(self):
        dataset_list = simulate_choice_dataset.simulate_dataset_v2(
            num_users=num_users,
            num_items=num_items,
