...
def simulate_dataset_v2(
    ...
    # generate 12 features for each session user pair, e.g., the budget of
    # that user at the shopping day.
    itemsession_obs = torch.randn(num_sessions, num_items, 12)
    # generate 12 features for each user item pair, e.g., the user's
    # preference on that item.
    useritem_obs = torch.randn(num_users, num_items, 12)
    # generate 10 user-session specific observables, e.g., the historical
    # spending amount of that user at that session.
    ...
    # since `U*S*I` is potentially huge and may cause identifiability issues,
    # we rarely use this kind of observable in practice.
