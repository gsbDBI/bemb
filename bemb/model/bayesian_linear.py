...
class BayesianLinear(nn.Module):
    def __init__(self,
        ...
        # the same as latent dimension before.
        ...
        # ==============================================================================================================
        # the prior of weights are gausssian distributions independent across
        # in_feature dimensions.
        self.register_buffer(
            'W_prior_mean', torch.zeros(
        ...
        self.register_buffer(
            ...
            torch.ones(
                in_features,
                out_features) *
            np.log(W_prior_variance))

        if self.bias:
            self.register_buffer(
                'b_prior_mean', torch.zeros(
                    in_features, out_features))
            self.register_buffer(
                'b_prior_logstd',
                torch.ones(
        ...
        if W_variational_mean_fixed is None:
            ...
            self.register_buffer(
                'W_variational_mean_fixed',
                W_variational_mean_fixed)

        # TODO: optionally add customizable initialization here.
        self.W_variational_mean_flexible = nn.Parameter(
            torch.randn(in_features, out_features), requires_grad=True)
        self.W_variational_logstd = nn.Parameter(
            torch.randn(in_features, out_features), requires_grad=True)

        if self.bias:
            self.b_variational_mean = nn.Parameter(
                torch.randn(out_features), requires_grad=True)
            self.b_variational_logstd = nn.Parameter(
                torch.randn(out_features), requires_grad=True)
    ...
    def rsample(self,
                num_seeds: int = 1) -> Optional[Tuple[torch.Tensor,
                                                      Optional[torch.Tensor]]]:
        """sample all parameters using re-parameterization trick.
        """
        self.num_seeds = num_seeds
        self.W_sample = self.W_variational_distribution.rsample(
            torch.Size([num_seeds]))

        if self.bias:
            self.b_sample = self.b_variational_distribution.rsample(
                torch.Size([num_seeds]))
    ...
    def forward(self, x, mode: str = 'multiply'):
        ...
        if mode == 'multiply':
            # (num_seeds, N, in_features)
            x = x.view(1, -1, self.in_features).expand(self.num_seeds, -1, -1)
    ...
    def W_variational_distribution(self):
        ...
        return Normal(
            loc=self.W_variational_mean,
            scale=torch.exp(
                self.W_variational_logstd))

    @property
    def b_variational_distribution(self):
        return Normal(
            loc=self.b_variational_mean,
            scale=torch.exp(
                self.b_variational_logstd))
    ...
    def log_variational(self):
        ...
        total_log_prob = torch.zeros(num_seeds, device=self.device)
        total_log_prob += self.W_variational_distribution.log_prob(
            self.W_sample).sum(
            dim=[
                1,
                2])
        if self.bias:
            total_log_prob += self.b_variational_distribution.log_prob(
                self.b_sample).sum(dim=1)
        assert total_log_prob.shape == (num_seeds,)
        return total_log_prob
