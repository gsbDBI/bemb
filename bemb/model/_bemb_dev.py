...
class BEMBFlex(nn.Module):
    def __init__(self,
        ...
        cprint('You are using BEMB Flex v3 (dev)', 'red')
        ...
        self.asymmetric_label = asymmetric_label
        ...
        self.register_buffer(
            'category_to_item_tensor',
            category_to_item_tensor.long())
        self.register_buffer(
            'category_to_size_tensor',
    ...
    def forward(self, batch, return_logit: bool = False,
                all_items: bool = True) -> torch.Tensor:
        ...
        # NOTE: here we don't need to sample the obs2prior weight H since we
        # only compute the log-likelihood.
        sample_dict = dict()
        ...
        if all_items:
            ...
            out = self.log_likelihood_all_items(
                batch, sample_dict, return_logit)
        else:
    ...
    def log_likelihood(self,
        ...
        for module in self.additional_modules:
            ...
            assert additive_term.shape == (
                R, len(batch)) or additive_term.shape == (
                R, len(batch), 1)
    ...
    def compute_log_likelihood(self, batch, sample_dict):
        if self.asymmetric_label:
            # get utility.
            u = self.log_likelihood(batch, sample_dict, return_logit=True)
            num_seeds = u.shape[0]
            # create copies of the label for computing.
            y = torch.stack([batch.asymmetric_label] * num_seeds).float()
            loss = nn.BCELoss(reduction='none')
            ll = -loss(torch.sigmoid(u), y).sum(dim=1).mean(dim=0)
            return ll
        else:
    ...
    def elbo(self, batch, num_seeds: int = 1) -> torch.Tensor:
        ...
        elbo += self.compute_log_likelihood(batch, sample_dict)

        # self.log_likelihood(batch, sample_dict).sum(dim=1).mean(dim=0)

        # if make binary choice.
        # NOTE & TODO: make is more consistent.
        # special case while making binary choice.
        # (num_sessions,) P(correct).
        # z = self.log_likelihood(batch, sample_dict, return_logit=True)
        # num_seeds = z.shape[0]
        # # create copies of the label for computing.
        # y = torch.stack([batch.asymmetric_label] * num_seeds).float()
        # loss = nn.BCELoss(reduction='none')

        # ll = -loss(torch.sigmoid(z), y).mean(dim=0).sum()
        # elbo += ll
