...
class AdditiveComponent(nn.Module):
    def __init__(self, formula: str,
                 num_classes: int,
                 dim: int):
        self.encoder = BayesianLinear(
            in_features=num_classes, out_features=dim)
...
class BEMB(nn.Module):
    def __init__(self,
        ...
        """
        super().__init__()
        cprint('You are using BEMB example version', 'red')
        ...
        if preprocessing_modules is not None:
            self.preprocessing_modules = nn.ModuleList(preprocessing_modules)
