...
(Version 4) Adding customized plug-in.
...
class BEMBFlex(nn.Module):
    def __init__(self,
                 ...
                 additive_modules: List[object] = None,  # TODO: add datatype.
        ...
        cprint('You are using BEMB Flex v4 (beta)', 'red')
        ...
        if additive_modules is not None:
            self.additive_modules = nn.ModuleList(additive_modules)
    ...
    def log_likelihood_all_items(self,
        ...
        # ==============================================================================================================
        # Execute additional non-standard customized utility modules.
        # ==============================================================================================================

        if self.additive_modules is not None:
            for module in self.additive_modules:
                additive_term = module(batch)
                assert additive_term.shape == (R, len(batch), I)
                utility += additive_term
    ...
    def log_likelihood(self,
        ...
        # argwhere(reverse_indices == k) are positions in relevant_item_indexs
    ...
    def __check_log_likelihood(self, batch, sample_dict):
        ...
        for return_logit in (True, False):
            A = self.log_likelihood(
                batch, sample_dict, return_logit=return_logit)

            B = self.log_likelihood_all_items(
                batch, sample_dict, return_logit=return_logit)
            B = B[:, torch.arange(len(batch)), batch.label]

            C = self.__log_likelihood_all_items_old(
                batch, sample_dict, return_logit=return_logit)
            C = C[:, torch.arange(len(batch)), batch.label]

            assert torch.max(
                torch.abs(
                    A - B)) < 1E-5, f'{torch.max(torch.abs(A - B))}'
            assert torch.max(
                torch.abs(
                    B - C)) < 1E-5, f'{torch.max(torch.abs(B - C))}'
