...
class LogitDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        self.item_sess_price = pd.read_csv(
            os.path.join(
                data_path,
                'item_sess_price.tsv'),
            sep='\t',
        ...
        self.availability_data = pd.read_csv(
            os.path.join(
                data_path,
                'availabilityList.tsv'),
            sep='\t',
            header=None,
            names=columns,
            dtype=str)
        self.availability_data = self.availability_data.merge(
            self.item_group, on='item_id', how='left')
        ...
        self.joint_training_data = self.purchase.merge(
            self.item_group, on='item_id', how='left')
        ...
        if self.category_we_care is not None:
            ...
            self.category_we_care = self.curr_train_data['category_id'].unique(
            ).values

        # add calendar day information associated with session.
        self.curr_training_data = self.curr_training_data.merge(
            self.sess_days, on='date', how='left')
    ...
    def report_stats(self):
        ...
        print(
            'number of dates (calendar date X store ID): ',
            df['date'].nunique())
        print('number of session (user X date): ',
    ...
    def get_all_data(self) -> Tuple[torch.Tensor]:
        ...
        # ['date', 'item_id', 'availability']
        self.availability_data['availability'] = 1

        a_wide = self.availability_data.pivot(
        ...
        obs_user = self.curr_training_data.set_index('user_id').join(
            self.obs_user.set_index('user_id'), how='left').reset_index()
        obs_user = self.curr_training_data.merge(
            self.obs_user, how='left', on='user_id')
        ...
        month_indicator, week_indicator, day_indicator = map(
            np.array, (month_indicator, week_indicator, day_indicator))
        ...
        return (
            constants.astype(
                np.float64), obs_user.astype(
                np.float64), prices, item_onehot.astype(
                np.float64), month_indicator.astype(
                    np.float64), day_indicator.astype(
                        np.float64), week_indicator.astype(
                            np.float64), A)
    ...
    def __getitem__(self, index):
        ...
        item_month = self.curr_training_data['date'].map(
            lambda x: int(str(x)[-4:-2])).values[index]
        if self.all_availabilities[index][np.argmax(
                self.all_item_onehot[index])] < 0.5:
