...
from _deepchoice.data import utils
...
def main(args) -> ChoiceDataset:
    ...
    price_cost = utils.pivot3d(
        mode_canada, 'case', 'alt', [
            'cost', 'freq', 'ovt'])
    session_income = torch.Tensor(mode_canada.groupby(
        'case')['income'].first().values).view(-1, 1)
    price_ivt = utils.pivot3d(mode_canada, 'case', 'alt', 'ivt')

    label = mode_canada.pivot('case', 'alt', 'choice')[
        ['air', 'bus', 'car', 'train']].values
