...
def generate_gaussian_features(
    ...
    low = - num_features // 2
    mu_lst = np.random.choice(
        np.arange(
            low,
            low + num_features),
        size=num_features,
        replace=False)
    sigma_lst = np.random.choice(
        np.arange(
            1,
            num_features + 1),
        size=num_features,
...
def generate_Xy(user_sess, user_feat, item_feat):
    ...
    X_user = user_feat[user_sess, :].view(
        num_sessions, 1, num_user_features).expand(-1, num_items, -1)
    # cast item features to all sessions.
    X_item = item_feat.view(
        1, num_items, num_item_features).expand(
        num_sessions, -1, -1)

    X = {'u': X_user, 'i': X_item}

    coef = dict()
    # user-specific features have coefficients varying across items. (type =
    # item_full)
    num_coefs = num_items * num_user_features
    coef['u'] = torch.arange(-num_coefs // 2, -num_coefs //
                             2 + num_coefs).view(1, num_items, num_user_features)
    ...
    c = torch.arange(-num_coefs // 2, -num_coefs // 2 +
                     num_coefs).view(num_users, num_item_features)
    c = c[user_sess, :].view(
        num_sessions, 1, num_item_features).expand(-1, num_items, -1)
    coef['i'] = c

    utility = (X['u'] * coef['u']).sum(dim=-1) + \
        (X['i'] * coef['i']).sum(dim=-1)

    noise = np.random.gumbel(
        size=(num_sessions, num_items)) * float(utility.std() / 10)
    utility += noise
...
def generate_dataset(num_users: int,
    ...
    user_sess = torch.Tensor(
        np.random.randint(
            low=0, high=num_users, size=(
                num_sessions,))).long()
