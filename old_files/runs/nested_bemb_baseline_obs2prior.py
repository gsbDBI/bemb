...
from deepchoice.model import NestedBEMB
...
if __name__ == '__main__':
    ...
    for d in (train, validation, test):
        ...
        choice_dataset_item = ChoiceDataset(label=label,
    ...
    model = NestedBEMB(num_users=configs.num_users,
                       num_items=configs.num_items,
                       obs2prior_dict_item=configs.obs2prior_dict_item,
                       obs2prior_dict_category=configs.obs2prior_dict_category,
                       latent_dim_item=configs.latent_dim_item,
                       latent_dim_category=configs.latent_dim_category,
    ...
    for i in tqdm(range(configs.num_epochs)):
        ...
        for batch in dataloaders['train']:
            # maximize the ELBO.
            loss = - model.elbo(batch_item=batch['item'], batch_category=batch['category'],
                                num_seeds=1)
