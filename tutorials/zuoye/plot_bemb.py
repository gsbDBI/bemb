...
group = dict(zip(range(NUM_QUESTIONS), [0] * (NUM_QUESTIONS//2) + [1] * (NUM_QUESTIONS//2)))
...
def plot_latent(state_dict):
    for coef_name in ['alpha_item']:
        ...
        norm = torch.sqrt(L.norm(dim=1).view(-1, 1) * L.norm(dim=1).view(1, -1))
        data = L @ L.T / norm
        # data = L @ L.T
    ...
    sns.scatterplot(x=X_embedded[:, 0], y=X_embedded[:, 1], ax=ax, c=color, alpha=1.0, s=5)
